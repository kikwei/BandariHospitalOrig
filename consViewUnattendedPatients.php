    ...
    <form action="consAttendPatient.php" method="GET">
        <table border='1' align='center' class='table table-hover text-center' id="myTable">
            ...
            if(!isset($_SESSION['logInConsultant'])){
                ...
                $query="SELECT * FROM PATIENTS WHERE STATUS='UNATTENDED' ORDER BY DateOfRegistration ASC ";
                ...
                while($rows=mysqli_fetch_array($queryResult)){
                    ...
                    echo "<tr><td><input type='radio' value='$rows[5]' name='patientsId'> </td><td>$rows[0]</td><td>$rows[1]</td><td>$rows[2]</td><td>$rows[3]</td><td>$rows[4]</td><td>$rows[5]</td><td>$rows[6]</td><td>$rows[7]</td><td>$rows[8]</td><td>$rows[9]</td><td>$rows[10]</td><td>$rows[11]</td><td>$rows[12]</td><td>$rows[13]</td></tr>";
